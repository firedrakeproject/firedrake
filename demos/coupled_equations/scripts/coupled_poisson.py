...
def build_problem(mesh1, mesh2):
    ...
    b1 = inner(f1, v1) * dx1
    b2 = inner(f2, v2) * dx2
    A = A11_form + A22_form + A12_form + A21_form
    L = b1 + b2
...
# PETSc.Sys.Print(f"...")
    ...
    plt.loglog(h2_array, errors_2, "o-", label="Poisson_2")
    plt.loglog(h1_array, errors_1, "s-", label="Poisson_1")
    plt.xlabel("h")
    plt.ylabel("L2 error")
    plt.gca().invert_xaxis()
    plt.grid(False)
    plt.legend()
    plt.title("Poisson-Poisson Coupling with Dirichlet-Neumann BCs")
