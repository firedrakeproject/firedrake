...
the spatial structure and frequencies of the freely evolving modes in this system, what are referred at as basin modes.
...
solve the QG eigenvalue problem with no basic state and no dissipation.
forces.
Unlike the other demo that integrated the equations forward in time, in this
probelm it is necessary to compute the eigenvalues and eigenfunctions
for a particular differential operator. This requires using PETC matrices
and eigenvalues solvers in SLEPc.


Governing PDE
=============

We will first present the nonlinear, one-layer QG equation that was used
in the other tutorial (link). The interested reader can find the
derivations in Pedlosky:1992 and Vallis:2006.
...
velocity field, q is the Potential Vorticity (PV), :math:`\beta` is the
Coriolis parameter and F is the rotational Froude number. The velocity
...
and replaceing it with an eigenvalue, :math:`i \omega`, By substituting
...
the test functions inheret these boundary condition,
...
The two non-dimensinal parameters are the :math:`\beta` parameter, set
