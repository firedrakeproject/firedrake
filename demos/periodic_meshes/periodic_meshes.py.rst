...
The two important arguments are ``layers`` and ``layer_height``, which tell Firedrake how far to extrude the mesh. By default ``layer_height`` is 1/``layers``. ::
    ...
    print(f"L2 error = {errornorm(u_exact, uh, norm_type='L2'):.3e}")
    VTKFile("output/cylinder_poisson.pvd").write(uh)
...
ends, recovering periodicity along the axis. 

We take the axial (toroidal) coordinate to run over :math:`[0, 2\pi)`
and identify the two end caps by a translation of :math:`2\pi R_0` along ``Z``. 
:math:`R_0` is called the major radius and ensures that the torus that the cylinder defines is the correct length. ::
...
merge intact -- the volume of the cylinder is :math:`\pi r^2 h = 2\pi^2 R0` -- while the ends carry no exterior
facets::

    volume = assemble(Constant(1.0) * dx(domain=msh))
    print(
        f"cylinder volume: {volume:.4f}  "
        f"(exact 2*pi**2*R0 = {2*pi**2*R0:.4f})"
    )
...
.. math::
   ...
   u_{\text{ex}}(x, y, z) = \cos(z/R_0) \bigl(1 - x^2 - y^2\bigr),
...
We plot the output in :math:`(R, \phi, Z)`, a cylindrical representation:
