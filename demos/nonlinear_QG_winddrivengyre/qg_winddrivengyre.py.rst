...
Now we will define all the parameters we are using in this tutorial. ::
  ...
  beta = Constant('1.0', domain=mesh)      # Beta parameter
  F = Constant('1.0', domain=mesh)         # Burger number
  r = Constant('0.2', domain=mesh)         # Bottom drag
  tau = Constant('0.001', domain=mesh)     # Wind Forcing
