...
cycles.  ``dwr_rtol`` stops the loop when the estimated error in the goal falls
below that fraction of the absolute value of the current goal functional,
evaluated on the uncorrected, non-enriched solution.  ``dwr_atol`` supplies an
...
The DWR callback solves the low- and enriched-order dual problems, localises
