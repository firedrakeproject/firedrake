        ...
        stages {
          ...
          stage('Build') {
            ...
            steps {
              ...
              dir('tmp') {
                timestamps {
                  sh '../scripts/firedrake-install $COMPLEX --package-branch fiat bdmc --package-branch tsfc bdmc --package-branch FInAT bdmc --tinyasm --disable-ssh --minimal-petsc --slepc --documentation-dependencies --install thetis --install gusto --install icepack --install irksome --no-package-manager || (cat firedrake-install.log && /bin/false)'
                }
              }
            }
          }
