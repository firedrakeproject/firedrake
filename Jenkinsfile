  ...
  stages {
    ...
    stage('Build') {
      steps {
        ...
        dir('tmp') {
          timestamps {
            sh '../scripts/firedrake-install --disable-ssh --minimal-petsc ${SLEPC} --adjoint --slope --install thetis --install gusto --install pyadjoint ${PACKAGE_MANAGER} --package-branch ufl complex --package-branch tsfc complex || (cat firedrake-install.log && /bin/false)'
          }
        }
      }
    }
