  ...
  stages {
    ...
    stage('Build') {
      steps {
        ...
        dir('tmp') {
          timestamps {
            sh '../scripts/firedrake-install --tinyasm --disable-ssh --minimal-petsc --slepc --documentation-dependencies --install thetis --install gusto --install icepack --install irksome --no-package-manager --package-branch firedrake fs_filter --package-branch PyOP2 fs_filter --package-branch ufl transformed_form_argument --package-branch tsfc c626db38f3cbe22c01d9de7255eec72a676f0059 --package-branch fiat add_entity_ids_by_differentiation_order --package-branch FInAT add_entity_ids_by_differentiation_order || (cat firedrake-install.log && /bin/false)'
          }
        }
      }
    }
