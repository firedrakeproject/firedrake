        ...
        stages {
          ...
          stage('Build') {
            ...
            steps {
              ...
              dir('tmp') {
                timestamps {
                  sh '../scripts/firedrake-install $COMPLEX --disable-ssh --minimal-petsc --slepc --documentation-dependencies --install thetis --install gusto --install icepack --no-package-manager --package-branch firedrake complex-sprint --package-branch pyadjoint complex-sprint || (cat firedrake-install.log && /bin/false)'
                }
              }
            }
          }
