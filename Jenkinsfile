        ...
        stages {
          ...
          stage('Build') {
            ...
            steps {
              ...
              dir('tmp') {
                timestamps {
                  sh '../scripts/firedrake-install $COMPLEX --tinyasm --disable-ssh --minimal-petsc --slepc --documentation-dependencies --install thetis --install gusto --install icepack --install irksome --no-package-manager --package-branch loopy firedrake-fix_validate_kc_sites|| (cat firedrake-install.log && /bin/false)'
                }
              }
            }
          }
