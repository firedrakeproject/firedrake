  ...
  stages {
    ...
    stage('Build') {
      steps {
        ...
        dir('tmp') {
          timestamps {
            sh '../scripts/firedrake-install --disable-ssh --minimal-petsc --slepc --documentation-dependencies --install thetis --install gusto --install icepack --install pyadjoint --package-branch fiat hdivt-grad-nan --no-package-manager || (cat firedrake-install.log && /bin/false)'
          }
        }
      }
    }
