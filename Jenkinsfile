  ...
  stages {
    ...
    stage('Build') {
      steps {
        ...
        dir('tmp') {
          timestamps {
            sh '../scripts/firedrake-install --disable-ssh --minimal-petsc --slepc --documentation-dependencies --install thetis --install gusto --install icepack --install pyadjoint --no-package-manager --package-branch ufl external-operator --package-branch tsfc pointwise-operators || (cat firedrake-install.log && /bin/false)'
          }
        }
      }
    }
