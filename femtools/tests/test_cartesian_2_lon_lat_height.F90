...
!    USA
subroutine test_cartesian_2_lon_lat_height
  !Subroutine for testing correct conversion of point coordinates from Cartesian
  ! into longitude-latitude-height coordinates.
  use fields
  use vtk_interfaces
  use state_module
  use Coordinates
  ...
  !Extract the components of points in vtu file in Cartesian, apply transformation and
  ! compare with position-vector in lon-lat-radius coordinates.
  call allocate(difference, 3 , mesh, 'difference')
  do node=1,node_count(CartesianCoordinate)
    XYZ = node_val(CartesianCoordinate, node)
    call cartesian_2_lon_lat_height(XYZ(1), XYZ(2), XYZ(3), &
                                    LLH(1), LLH(2), LLH(3), &
                                    0.0)
  ...
  call report_test("[Coordinate change: Cartesian to lon-lat-height.]", &
