...
def get_injection_matrix_kernel(fiat_element, unique_indices, dim=1):
    ...
    weights = gem.Literal(weights.T)
    A = gem.Variable("A", (nfdof, dim, ncdof, dim))
    i = gem.Index("i")
    j = gem.Index("j")
    k = gem.Index("k")
    l = gem.Index("l")
    lvalue = gem.Indexed(A, (i, k, j, l))
    rvalue = gem.Indexed(weights, (i, j))
    body, _ = driver.build_kernel_body([lvalue], [rvalue], (i, k, j, l))
    arglist = [ast.Decl("double", ast.Symbol("A", rank=(nfdof, dim, ncdof, dim)))]

    kernel = ast.FunDecl("void", "injection_matrix", arglist,
                         body,
                         pred=["static", "inline"])

    return op2.Kernel(kernel, kernel.name)


def get_prolongation_matrix_kernel(fiat_element, unique_indices, dim=1):
    from tsfc import gem, driver
    weights = gem.Literal(get_restriction_weights(fiat_element)[unique_indices])
    ...
    kernel = ast.FunDecl("void", "prolongation_matrix", arglist,
                         body,
                         pred=["static", "inline"])

    return op2.Kernel(kernel, kernel.name, opts=parameters["coffee"])
