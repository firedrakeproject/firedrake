...
def MeshHierarchy(mesh, refinement_levels=0,
    ...
    netgen_flags : bool, dict
        options for the Netgen geometry attached to ``mesh``. The only
        option read here is ``degree``: either an integer, or a sequence
        with one entry per mesh in the hierarchy, giving the polynomial
        degree of the coordinates of each level. It defaults to the degree
        of the coordinates of ``mesh``.
    ...
    Notes
    -----
    If ``mesh`` carries a geometry source, such as the Netgen geometry of a
    mesh built from a Netgen mesh, then it is attached to every level, and
    the coordinates of each level are re-evaluated on that geometry.

    """
    if netgen_flags is True or isinstance(netgen_flags, dict):
        utils.check_netgen_installed()
        if mesh._geometry_source is None:
            raise RuntimeError("Cannot pass netgen_flags to a mesh that has not "
                               "been generated by Netgen.")
    else:
        netgen_flags = {}

    # The coordinate degree of each mesh in the hierarchy.
    nmeshes = refinement_levels*refinements_per_level + 1
    coarse_degree = mesh.coordinates.function_space().ufl_element().degree()
    degree = netgen_flags.get("degree", coarse_degree)
    if isinstance(degree, int):
        degree = (degree,)*nmeshes
    degree = tuple(degree)
    if len(degree) != nmeshes:
        raise ValueError(f"Expecting one coordinate degree per mesh in the "
                         f"hierarchy ({nmeshes}), got {len(degree)}")

    geometry_source = mesh._geometry_source
    if geometry_source is not None:
        if degree[0] != coarse_degree:
            mesh = firedrake.Mesh(mesh.curve_field(degree[0]))
    ...
    if refinement_levels > 0:
        ...
        if geometry_source is not None and degree[0] > 1:
            # Refinement, and conversion back to Netgen, both require the
            # straight-sided cells of a linear coordinate discretization.
            from firedrake.netgen import _linearize_coordinate_dm
            _linearize_coordinate_dm(cdm)
    ...
    meshes = [mesh]
    for rdm, order in zip(dms[1:], degree[1:]):
        fmesh = mesh_builder(
            rdm,
        ...
        if geometry_source is not None:
            fmesh = geometry_source.recurve(fmesh, order)
        meshes.append(fmesh)
