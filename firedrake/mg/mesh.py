...
from .utils import get_level, has_level, set_level
...
def MeshHierarchy(mesh, refinement_levels,
    ...
    tdim = cdm.getDimension()
    num_refinements = refinement_levels*refinements_per_level
    if mesh.submesh_parent is not None and has_level(mesh.submesh_parent):
        # Deal with Submesh
        parent_hierarchy, level = get_level(mesh.submesh_parent)
        start = level * refinements_per_level
        if len(parent_hierarchy._meshes) < start+num_refinements+1:
            raise ValueError("The parent hierarchy is not sufficiently refined")
        # re-apply the same label filter to each level of the parent hierarchy
        submesh_parents = parent_hierarchy._meshes[start:start+num_refinements+1]
        # The parent hierarchy is already distributed, so there is no need to redistribute
        distribution_parameters = firedrake.mesh.DISTRIBUTION_PARAMETERS_NOOP
        ignore_halo = False
        label_name = mesh._submesh_label_name
        subdomain_id = mesh._submesh_subdomain_id
        comm = cdm.comm
        dms = [cdm]
        for submesh_parent in submesh_parents[1:]:
            parent_dm = submesh_parent.topology_dm
            subdm = dmcommon.submesh_create(parent_dm, tdim, label_name, subdomain_id, ignore_halo, comm=comm)
            dms.append(subdm)
    else:
        ...
        cdm = dmcommon.submesh_create(cdm, tdim, "depth", tdim, True)
