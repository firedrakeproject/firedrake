...
class MeshHierarchy(object):
    def __init__(self, m, refinement_levels, refinements_per_level=1, reorder=None):
        ...
        for i in range(refinement_levels*refinements_per_level):
            ...
            if hasattr(m, '_circle_manifold_radius'):
                coords = cdm.getCoordinatesLocal().array.reshape(-1, 2)
                scale = m._circle_manifold_radius / np.linalg.norm(coords, axis=1).reshape(-1, 1)
                coords *= scale
            elif hasattr(m, '_icosahedral_sphere_radius'):
                coords = cdm.getCoordinatesLocal().array.reshape(-1, 3)
                scale = m._icosahedral_sphere_radius / np.linalg.norm(coords, axis=1).reshape(-1, 1)
                coords *= scale
