...
class Assigner:
    ...
    def _assign_via_sections(self, lhs_func, subset, funcs, point_sf,
        ...
        The nodes the two spaces have in common are in one-to-one
        correspondence, so the expression is evaluated in the source layout and
        the result is moved into the target layout with a single communication.
        ...
        root by reduction, only the latter being able to miss nodes.
        """
        target_V = lhs_func.function_space()
