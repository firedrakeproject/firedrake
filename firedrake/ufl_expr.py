...
class Argument(ufl.argument.Argument):
    ...
    #    # when assembling.
    #    # This 'test' function may or may not have number = 0.
...
def derivative(form, u, du=None, coefficient_derivatives=None):
    ...
    #if not u_is_x and len(u.split()) > 1 and not u.mixed() and set(extract_coefficients(form)) & set(u.split()):
    ...
    mesh = form.ufl_domains()
    coords_tuple = tuple(m.coordinates for m in mesh)
    if u_is_x or u in coords_tuple:
        if u_is_x:
            coords = u.ufl_domain().coordinates
        else:
            coords = u
            u = ufl.SpatialCoordinate(mesh[coords_tuple.index(u)])
        ...
        V = firedrake.FunctionSpace(u.ufl_domain(), "Real", 0)
        du = argument(V)
    else:
