...
def submesh_create_sub_super_map(submesh, dim):
    ...
    # Check dim
    if dim > submesh._topology_dm.getDimension():
    ...
    # Fill sub_super_map
    subpStart, subpEnd = submesh._topology_dm.getDepthStratum(dim)
    sub_super_map = np.empty((subpEnd - subpStart, 1), dtype=IntType)

    codim = mesh._topology_dm.getDimension() - dim
    subcodim = submesh._topology_dm.getDimension() - dim
    ...
    subpointindices = submesh._topology_dm.getSubpointIS().getIndices()
