...
# Minimal PETSc declarations — only what this module uses.
...
# MPI_INT is a C runtime value; declare it directly from the MPI header.
cdef extern from "mpi.h" nogil:
    MPI.MPI_Datatype MPI_INT
...
def build_from_aabb(np.ndarray[np.float64_t, ndim=2, mode="c"] regions_lo,
    ...
    ids : (n,) array, optional
        Integer ids for each box. If not provided, defaults to 0-based indices.
    ...
    rstar_tree = RTree(<uintptr_t>rtree)
    return rstar_tree
...
def discover_ranks(
    ...
    For each point in `points`, finds all candidate ranks whose bounding
    ...
    # send_offsets[i]   = start index in point_indices for toranks[i]
    # send_offsets[nto] = total number of (point, rank) pairs
