...
class PlaneSmoother(object):
    ...
    def sort_entities(self, dm, axis, dir, ndiv=None, divisions=None):
        ...
        ele = mesh_unique.coordinates.function_space().ufl_element()
        V = mesh_unique.coordinates.function_space()
        if V.finat_element.entity_dofs() == V.finat_element.entity_closure_dofs():
            ...
            CGk = FunctionSpace(mesh_unique.coordinates.function_space().mesh(), CGkele)
            coordinates = Interpolate(mesh_unique.coordinates, CGk, access=op2.MAX)
