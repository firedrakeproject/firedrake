...
from firedrake.adjoint import PETScVecInterface, check_rf_action, TLMAction, \
                                AdjointAction, HessianAction, get_valid_comm, \
                                flatten_parameters, ReducedFunctionalMatCtx
from firedrake import Function, Cofunction, TestFunction, TrialFunction, solve, inner, dx, assemble
...
def new_restricted_control_variable(reduced_functional, function_space, dual=False):
    ...
    return tuple(Function(function_space).interpolate(control.control)._ad_init_zero(dual=dual)
...
def interpolate_vars(variables, function_space):
    ...
    if isinstance(variables, (tuple, list)):
        if isinstance(variables[0], Function):
            ...
            return tuple(Cofunction(function_space.dual()).interpolate(v) for v in variables)
...
class RestrictedReducedFunctionalMatCtx(ReducedFunctionalMatCtx):
    ...
    def __init__(self, rf, action=HessianAction, *,
        ...
        if restricted_space is None:
            raise ValueError("restricted_space must be provided for RestrictedReducedFunctionalMatCtx")

        super().__init__(rf, action=action, apply_riesz=apply_riesz,
                         appctx=appctx, comm=comm)
        ...
        self.restricted_control_interface = (
            PETScVecInterface([Function(self.restricted_space).interpolate(c.control)
                               for c in rf.controls])
            if restricted_space else self.control_interface
        )
        ...
        if action == HessianAction:
            ...
            self.yresinterface = self.restricted_control_interface
            self.xres = Function(self.restricted_space).interpolate(rf.functional)._ad_copy()._ad_init_zero(dual=True)
            self.yres = new_restricted_control_variable(rf, self.restricted_space)
            ...
            self.xres = new_restricted_control_variable(rf, self.restricted_space)
            self.yres = Function(self.restricted_space).interpolate(rf.functional)._ad_copy()._ad_init_zero(dual=True)
...
def RestrictedReducedFunctionalMat(rf, action=HessianAction, *, apply_riesz=False, appctx=None, comm=None, restricted_space=None):
    """
