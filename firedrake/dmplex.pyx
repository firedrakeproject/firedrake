...
def mark_entity_classes(PETSc.DM plex, s_depth=1):
    ...
    # Set DMPlex adjancency definition to cell adjacency,
    # so that adjacency(c) := star(closure(c))
    plex.setAdjacencyUseCone(useCone=True)
...
def get_facets_by_class(PETSc.DM plex, label,
    ...
    """
    cdef:
        PetscInt dim, fi, ci, s, nfacets, nclass, lbl_val, f, fStart, fEnd
        PetscInt *indices = NULL
    ...
    for i, op2class in enumerate(["op2_core",
        ...
        if nclass > 0:
            for f in ordering:
                ...
                CHKERR(DMLabelStratumHasPoint(lbl_class, s, f, &is_class))
