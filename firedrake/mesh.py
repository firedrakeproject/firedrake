...
values from f.)"""
    ...
    def curve_field(self,
                    order: int,
                    permutation_tol: float = 1.e-8,
                    cg_field: bool | None = None) -> object:
        """Return a function containing re-evaluated mesh coordinates.

        This method requires a mesh construction source which supports
        coordinate re-evaluation, such as Netgen.

        Parameters
        ----------
        order
            Polynomial degree of the re-evaluated coordinates.
        permutation_tol
            Retained for compatibility. DMPlex coordinate elements do not use
            geometric permutation searches.
        cg_field
            Whether to return continuous coordinates. Defaults to the
            continuity of the current coordinates.

        Returns
        -------
        Function
            The re-evaluated coordinate field.

        """
        if self._geometry_source is None:
            raise ValueError(
                "This mesh has no geometry source capable of re-evaluating "
                "its coordinates."
            )
        return self._geometry_source.curve_field(
            self,
            order,
...
def make_mesh_from_coordinates(coordinates, name, tolerance=0.5):
    ...
    if isinstance(orig_mesh, MeshGeometry):
        mesh._geometry_source = orig_mesh._geometry_source
    return mesh
...
def Mesh(meshfile, **kwargs):
    ...
    * HDF5: with extension `.h5`, `.hdf5`
    ...
    from_netgen = user_comm.bcast(
        bool(netgen and isinstance(meshfile, netgen.libngpy._meshing.Mesh))
        if user_comm.rank == 0 else None,
        root=0,
    )
    ...
    if isinstance(meshfile, PETSc.DMPlex):
        ...
        from firedrake.netgen import NetgenGeometry

        petsctools.cite("Betteridge2024")
        netgen_flags = kwargs.get("netgen_flags", {})
        geometry_source = NetgenGeometry(meshfile, netgen_flags, user_comm)
        netgen_flags = geometry_source.options
        plex = geometry_source.plex
    ...
    topology = MeshTopology(plex, name=plex.getName(), reorder=reorder,
                            ...
                            comm=user_comm)
    if from_netgen and netgen_flags.get("degree", 1) != 1:
        from firedrake.netgen import _mesh_from_coordinate_dm

        mesh = _mesh_from_coordinate_dm(
            topology, name, netgen_flags["degree"]
        )
    else:
        mesh = make_mesh_from_mesh_topology(topology, name)

    if from_netgen:
        mesh._geometry_source = geometry_source
