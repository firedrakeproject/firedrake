...
class AbstractMeshTopology(metaclass=abc.ABCMeta):
    ...
    def __init__(self, name, reorder, sfXB, perm_is, distribution_name, permutation_name, comm):
        ...
        self._permutation_name = permutation_name
...
class MeshTopology(AbstractMeshTopology):
    ...
    @PETSc.Log.EventDecorator("CreateMesh")
    def __init__(self, name, reorder, distribution_parameters, sfXB=None, perm_is=None, distribution_name=None, permutation_name=None, comm=COMM_WORLD):
        """Initialise a mesh topology.
        ...
        super().__init__(name, reorder, sfXB, perm_is, distribution_name, permutation_name, comm)
...
class VertexOnlyMeshTopology(AbstractMeshTopology):
    ...
    def __init__(self, swarm, parentmesh, name, reorder, input_ordering_swarm=None, perm_is=None, distribution_name=None, permutation_name=None):
        ...
        super().__init__(name, reorder, None, perm_is, distribution_name, permutation_name, parentmesh.comm)
...
class MeshGeometry(ufl.Mesh, MeshGeometryMixin):
    ...
    def __init__(self, coordinates):
        ...
        if isinstance(topology, MeshTopology):
            ...
            assert isinstance(topology, VertexOnlyMeshTopology)
            self.geometry_dm = topology.topology_dm
    ...
    def _init_topology(self, topology, geometry_dm):
        ...
        def callback(self):
            ...
            if isinstance(self.topology, MeshTopology):
                # Add overlap to the mesh
                geometry_dm = self._input_geometry_dm
                sfBC = None
                grown_halos = False
                if self.comm.size > 1:
                    overlap_type, overlap = self.topology._distribution_parameters["overlap_type"]
                    sfBC, grown_halos = _add_overlap_to_dm(geometry_dm, overlap_type, overlap)
                self.topology._grown_halos = grown_halos

                if sfBC:
                    topology.sfBC = topology.sfBC.compose(sfBC) if topology.sfBC else sfBC
                if topology.sfXB is not None:
                    topology.sfXC = topology.sfXB.compose(topology.sfBC) if topology.sfBC else topology.sfXB

                # Finish the initialisation of mesh topology
                topology_dm = geometry_dm.clone()
                # effectively 'topology_dm.setCoordinates(NULL)'
                topology_dm.setCoordinates(geometry_dm.getCoordinateDM().getCoordinates())
            else:
                assert isinstance(self.topology, VertexOnlyMeshTopology)
                geometry_dm = self._input_geometry_dm
                topology_dm = self._input_geometry_dm

            self.topology.topology_dm = topology_dm
            self.topology.init()
...
def make_mesh_from_mesh_topology(topology, geometry_dm, name, tolerance=0.5):
    ...
    tdim = geometry_dm.getDimension()
    # Allow empty local meshes on a process
    cStart, cEnd = geometry_dm.getHeightStratum(0)  # cells
    if cStart == cEnd:
        nfacets = -1
    else:
        nfacets = geometry_dm.getConeSize(cStart)

    # TODO: this needs to be updated for mixed-cell meshes.
    nfacets = topology._comm.allreduce(nfacets, op=MPI.MAX)

    cell = ufl.Cell(_cells[tdim][nfacets])
    geometric_dim = geometry_dm.getCoordinateDim()
...
def make_vom_from_vom_topology(topology, dm, name, tolerance=0.5):
    ...
    vmesh._init_topology(topology, dm)
    ...
    if parent_tdim > 0:
        ...
        reference_coordinates_data = dmcommon.reordered_coords(dm, reference_coordinates_fs.dm.getDefaultSection(),
...
def Mesh(meshfile, **kwargs):
    ...
    * HDF5: with extension `.h5`, `.hdf5`
    ...
    # Create mesh topology - note that this is in a poorly defined
    # state until the mesh is finalised
    topology = MeshTopology(name=plex.getName(), reorder=reorder,
                            distribution_parameters=distribution_parameters,
    ...
    # Distribute the plex
    # from MeshTopology.__init__
    distribute = distribution_parameters.get("partition")
    if distribute is None:  # NOTE: Use a sane default in .get?
        distribute = True
    topology._distribution_parameters["partition"] = distribute
    topology._distribution_parameters["overlap_type"] = \
        distribution_parameters.get("overlap_type", (DistributedMeshOverlapType.FACET, 1))
    partitioner_type = distribution_parameters.get("partitioner_type")
    _set_dmplex_partitioner(plex, distribute, partitioner_type)
    topology._distribution_parameters["partitioner_type"] = plex.getPartitioner().getType()
    ...
    # from AbstractMeshTopology.__init__
    dmcommon.validate_mesh(plex)
    plex.setFromOptions()
    dmcommon.label_facets(plex)

    # Set/Generate names to be used when checkpointing.
    topology._distribution_name = topology._distribution_name or _generate_default_mesh_topology_distribution_name(user_comm.size, topology._distribution_parameters)
    topology._permutation_name = topology._permutation_name or _generate_default_mesh_topology_permutation_name(reorder)

    if user_comm.size > 1 and distribution_parameters["partition"]:
        ...
        # NOTE: plex carries a new dm after distribute, which
    ...
    # from callback
    dmcommon.label_facets(plex)
    dmcommon.complete_facet_labels(plex)

    if netgen and isinstance(meshfile, netgen.libngpy._meshing.Mesh):
        # for compat with netgen
        topology.topology_dm = plex
...
def VertexOnlyMesh(mesh, vertexcoords, reorder=None, missing_points_behaviour='error',
    ...
    topology.topology_dm = swarm
    vmesh_out = make_vom_from_vom_topology(topology, swarm, name, tolerance)
...
def _set_dmplex_partitioner(plex, distribute, partitioner_type=None):
