...
class InterpolateBlock(Block, Backend):
    ...
    f : W x G -> X -> V i.e. f(u, g) in V forall u in W and g in G.
                        X is infinite dimensional.
    f = I o expr
    I :   X -> V i.e. I(;x) in V forall x in X.
    expr: W x G -> X i.e. expr(u, g) in X forall u in W and g in G.
    ...
    def evaluate_adj_component(self, inputs, adj_inputs, block_variable, idx, prepared=None):
        ...
        # This calculates
        #
        # uhat = vhat . d_u[f](u, g; .) (for inputs[idx] in W)
        # or
        # ghat = vhat . d_g[f](u, g; .) (for inputs[idx] in G)
        #
        ...
        # inputs[idx] in W) or g' (for inputs[idx] in G) size (vhat left
        # multiplies the derivative).
        #
        # f = I o expr : W x G -> V
        #                i.e. I(expr|_{u, g}) in V forall u in W, g in G.
        #
        # Since I is linear we get that
        #
        # d_u[I o expr](u, g; u') = I o d_u[expr](u|_u, g|_g; u')
        # d_g[I o expr](u, g; g') = I o d_u[expr](u|_u, g|_g; g').
        #
        # In tensor notation
        #
