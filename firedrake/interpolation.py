...
def make_interpolator(expr, V, subset, access):
    ...
    if not isinstance(expr, firedrake.Expression):
        ...
        loops.extend(_interpolator(V, f.dat, expr, subset, arguments, access))
...
def _interpolator(V, dat, expr, subset, arguments, access):
    ...
    if not isinstance(expr, (firedrake.Expression, firedrake.SubExpression)):
        ...
        ast, oriented, needs_cell_sizes, coefficients, _ = compile_ufl_kernel(expr,
                                                                              to_pts,
                                                                              V.ufl_element(),
                                                                              coords,
                                                                              coffee=False)
        ...
        kernel, oriented, needs_cell_sizes, coefficients = compile_python_kernel(expr,
                                                                                 to_pts,
                                                                                 to_element,
                                                                                 V,
                                                                                 coords)
    ...
    if indexed:
        if isinstance(tensor, op2.Dat):
            args.append(tensor(access, V.cell_node_map()[op2.i[0]]))
        else:
            args.append(tensor(access, (V.cell_node_map()[op2.i[0]],
    ...
    if isinstance(tensor, op2.Mat):
        return copyin + (partial(op2.par_loop, *args), tensor.assemble()) + copyout
    else:
