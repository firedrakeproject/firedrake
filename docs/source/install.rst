...
``firedrake-configure``. On Ubuntu run::
  ...
  $ eval sudo apt install $(python3 firedrake-configure --show-system-packages)
...
If on macOS you should instead run::
  ...
  $ eval brew install $(python3 firedrake-configure --show-system-packages)
...
#. Run PETSc ``configure``, passing in the flags generated by ``firedrake-configure``::
   ...
   $ eval ./configure $(python3 ../firedrake-configure --show-petsc-configure-options)
...
to be downloaded and compiled from source::
   ...
   $ eval ./configure $(python3 ../firedrake-configure --no-package-manager --show-petsc-configure-options)
...
* Append additional options when ``configure`` is invoked. For example, to compile
  PETSc with 64 bit indices you should run::
   ...
   $ eval ./configure $(python3 ../firedrake-configure --show-petsc-configure-options) --with-64-bit-indices
...
#. Set any necessary environment variables. This can be achieved using
   ``firedrake-configure``::
     ...
     $ eval export $(python3 firedrake-configure --show-env)
