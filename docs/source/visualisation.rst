...
:class:`VTKFile <firedrake.vtk_output.VTKFile>` object. To create one, first import the
...
To save functions to the :class:`VTKFile <firedrake.vtk_output.VTKFile>` we use the
:meth:`write <firedrake.vtk_output.VTKFile.write>` method.
...
create the output :class:`VTKFile <firedrake.vtk_output.VTKFile>` outside the time loop
and call :meth:`write <firedrake.vtk_output.VTKFile.write>` inside.
...
:meth:`write <firedrake.vtk_output.VTKFile.write>`, by default an integer counter is
used that is incremented by 1 each time
:meth:`write <firedrake.vtk_output.VTKFile.write>` is called.  It is possible to
...
a common operation, the :class:`VTKFile <firedrake.vtk_output.VTKFile>` object is set up
...
.. note::
   ...
   Lagrange (L2) basis. The :class:`VTKFile <firedrake.vtk_output.VTKFile>` is also setup
...
:class:`function <firedrake.Function>`\s to :meth:`write <firedrake.vtk_output.VTKFile.write>`.
...
Vertex-only mesh data can be written to VTK output enabling point data visualisations in third-party software such as ParaView. A :class:`VTKFile <firedrake.vtk_output.VTKFile>` accepts :class:`function <firedrake.Function>`\s
