...
def test_scalar_convergence(extmesh, quadrilateral, testcase, convrate):
    ...
    for ii in [i + start for i in range(len(l2err))]:
        ...
        f = Function(fspace)
        f.interpolate(Expression((1+12*np.pi*np.pi)*cos(2*np.pi*x)*cos(2*np.pi*y)*cos(2*np.pi*z)))

        out = Function(fspace)
        solve(dot(grad(u), grad(v))*dx + u*v*dx == f*v*dx, out)

        exact = Function(fspace)
        exact.interpolate(Expression(cos(2*np.pi*x)*cos(2*np.pi*y)*cos(2*np.pi*z)))
