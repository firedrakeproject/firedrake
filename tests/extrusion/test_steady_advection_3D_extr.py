...
def test_right_to_left(mesh, V, W):
    velocity = Expression(("-1.0", "0.0", "0.0"))
    u0 = project(velocity, W)
...
def test_near_to_far(mesh, V, W):
    ...
    a1 = -D*dot(u0, grad(phi))*dx
    a2 = dot(jump(phi), un('+')*D('+') - un('-')*D('-'))*dS_v
    a3 = dot(phi, un*D)*ds_v(4)  # outflow at far wall
    a = a1 + a2 + a3
...
def test_far_to_near(mesh, V, W):
    ...
    a1 = -D*dot(u0, grad(phi))*dx
    a2 = dot(jump(phi), un('+')*D('+') - un('-')*D('-'))*dS_v
    a3 = dot(phi, un*D)*ds_v(3)  # outflow at near wall
    a = a1 + a2 + a3
