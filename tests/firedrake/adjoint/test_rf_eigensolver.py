...
from firedrake.adjoint import PETScVecInterface, AdjointAction
from firedrake.restricted_functional_ctx import new_restricted_control_variable, interpolate_vars
...
def test_compare_eigensolvers_helmholtz(expression):
    ...
    evals_form = [solver_form.eigenvalue(i) for i in range(0,min(num_eigenvalues, nconv))]
    ...
    evals_rf = [1/solver_rf.eigenvalue(i) for i in range(0,min(num_eigenvalues, nconv))]
...
def test_apply_riesz_param(restrict, riesz):
    ...
    problem_rf_true = RFEigenproblem(J, bcs=bc, apply_riesz=True, restrict=restrict, action="adjoint")
    problem_rf_false = RFEigenproblem(J, bcs=bc, apply_riesz=False, restrict=restrict, action="adjoint")
    ...
    interface_Ax = PETScVecInterface([Function(interface_space).interpolate(c.control)
                               for c in J.controls])
    func_representation = new_restricted_control_variable(J, interface_space, dual=False)
    func_representation_dual = new_restricted_control_variable(J, interface_space, dual=True)
    interface_Ax.from_petsc(Ax, func_representation)
    interface_Ax.from_petsc(Ax2, func_representation_dual)

    if restrict:
        #Intepolate from restricted to unrestricted
    ...
    set_working_tape(Tape())
    assert errornorm(func_representation[0], func_representation_dual[0].riesz_representation(riesz_map=riesz)) < 1e-6
...
def test_compare_tlm_adjoint():
    ...
    problem_rf_a = RFEigenproblem(J, bcs=bc, apply_riesz=False, restrict=True, identity=True, action="adjoint")
    ...
    evals_rf_a = [solver_rf_a.eigenvalue(i) for i in range(0,min(num_eigenvalues, nconv))]

    problem_rf_t = RFEigenproblem(J, bcs=bc, apply_riesz=False, restrict=True, identity=True, action="tlm")
    ...
    evals_rf_t = [solver_rf_t.eigenvalue(i) for i in range(0,min(num_eigenvalues, nconv))]
