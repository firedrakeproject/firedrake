...
from firedrake.adjoint import (
    ...
    set_working_tape, taylor_test, taylor_to_dict, Control,
    ReducedFunctional, AutoregressiveCovariance,
    WC4DVarReducedFunctional, SC4DVarReducedFunctional)
...
def strong_fdvar_firedrake(V):
    """Build an SC4DVarReducedFunctional for the strong constraint 4DVar system"""
    qn, qn1, stepper = timestepper(V)
...
def weak_fdvar_firedrake(V, ensemble):
    """Build an WC4DVarReducedFunctional for the weak constraint 4DVar system"""
    qn, qn1, stepper = timestepper(V)
    ...
    with Jhat.recording_stages() as stages:
        ...
        for stage, ctx in stages:
            ...
            # take observation
            obs_err = obs_errors(stage.global_index + 1)
