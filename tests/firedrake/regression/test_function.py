...
def test_function_assign_mixed_subset_3_quads_2_processes():
    ...
    V = CG1 * CG2
    cg1cg2 = Function(V)
    cg1, cg2 = cg1cg2.subfunctions
    cg1.interpolate(x)
    cg2.interpolate(x)
    # Include closure of the right-most cell.
    subset_cg1_indices = np.where(cg1.dat.data_ro_with_halos[:, 0] > 1.999)
    subset_cg2_indices = np.where(cg2.dat.data_ro_with_halos[:, 0] > 1.999)
    subset_cg1 = op2.Subset(CG1.node_set, subset_cg1_indices)
    subset_cg2 = op2.Subset(CG2.node_set, subset_cg2_indices)
    subset = op2.MixedSet([subset_cg1, subset_cg2])
    f = Function(V)
    c = Constant(7.)
    f.assign(c)
    f.assign(cg1cg2, subset=subset)
