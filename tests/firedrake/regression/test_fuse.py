...
@pytest.fixture
def mock_use_fuse(monkeypatch):
    monkeypatch.setenv("FIREDRAKE_USE_FUSE", "True")
...
@pytest.mark.parametrize(['params', 'degree', 'quadrilateral'],
                         [(p, d, q)
                          ...
                          for d in (1, 2, 3)
                          for q in [False, True]])
def test_poisson_analytic(mock_use_fuse, params, degree, quadrilateral):
...
def test_helmholtz(mock_use_fuse, conv_num, degree):
...
def test_helmholtz_3d(mock_use_fuse, conv_num, degree):
