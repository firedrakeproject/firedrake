...
def test_submesh_solve_cell_cell_mixed_scalar(dim, simplex):
    ...
    u0, u1 = split(u)
    v0, v1 = split(v)
    dx0 = Measure("dx", domain=mesh, additional_domain_integral_type_map={subm: "dx"})
    dx1 = Measure("dx", domain=subm, additional_domain_integral_type_map={mesh: "dx"})
...
def _mixed_poisson_solve_2d(nref, degree, quadrilateral, submesh_region):
    ...
    dx0 = Measure("dx", domain=mesh, additional_domain_integral_type_map={subm: "dx"})
    dx1 = Measure("dx", domain=subm, additional_domain_integral_type_map={mesh: "dx"})
    ds0 = Measure("ds", domain=mesh, additional_domain_integral_type_map={subm: "ds"})
    ds1_ext = Measure("ds", domain=subm, additional_domain_integral_type_map={mesh: "ds"})
    ds1_int = Measure("ds", domain=subm, additional_domain_integral_type_map={mesh: "dS"})
    dS0 = Measure("dS", domain=mesh, additional_domain_integral_type_map={subm: "ds"})
...
def _mixed_poisson_solve_3d(hexahedral, degree, submesh_region):
    ...
    dx1 = Measure("dx", domain=subm, additional_domain_integral_type_map={mesh: "dx"})
    ds0 = Measure("ds", domain=mesh, additional_domain_integral_type_map={subm: "ds"})
    ds1 = Measure("ds", domain=subm, additional_domain_integral_type_map={mesh: "dS"})
    bc = DirichletBC(W.sub(0), sigma_exact, boun_dirichlet)
    # Do the base case.
    w = Function(W)
    sigma, u = split(w)
...
def test_submesh_solve_cell_cell_equation_bc(nref, degree, simplex):
    ...
    dx_outer = Measure("dx", domain=mesh_outer, additional_domain_integral_type_map={mesh: "dx", mesh_inner: "dx"})
    dx_inner = Measure("dx", domain=mesh_inner, additional_domain_integral_type_map={mesh: "dx", mesh_outer: "dx"})
    ds_outer = Measure("ds", domain=mesh_outer, additional_domain_integral_type_map={mesh_inner: "ds"})
