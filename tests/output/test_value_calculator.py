...
from firedrake.plot import ValueCalculator
import numpy as np
...
def test_1d_constant():
    ...
    calculator = ValueCalculator(V, 10)
    f = Function(V)
...
def test_1d_linear():
    ...
    f.interpolate(2 * x)
    x_vals = calculator(mesh.coordinates)
    f_vals = calculator(f)
...
def test_2d_linear():
    ...
    coord_vals = calculator(mesh.coordinates).reshape(-1, 2)
    f_vals = calculator(f)
