...
def test_variable_layers_exterior_integrals(b1=0):
    ...
    np.testing.assert_allclose(assemble(Constant(1.0, domain=mesh)*ds_b(domain=mesh)), L)
    np.testing.assert_allclose(assemble(Constant(1.0, domain=mesh)*ds_t(domain=mesh)), sqrt(L**2+(H2-H1)**2))
    np.testing.assert_allclose(assemble(Constant(1.0, domain=mesh)*ds_v(1, domain=mesh)), H1)
    np.testing.assert_allclose(assemble(Constant(1.0, domain=mesh)*ds_v(2, domain=mesh)), H2)
