...
def test_all_dofs_on_facets():
    ...
    assert abs(assemble(dot(f, f)*dx) - assemble(dot(g, g)*dx)) < 6e-14
    assert abs(assemble(dot(f, f)*ds) - assemble(dot(g, g)*ds)) < 6e-14
    assert abs(assemble(dot(f, n)*ds) - assemble(dot(g, n)*ds)) < 6e-14
    assert abs(assemble(dot(f('+'), n('+'))*dS) - assemble(dot(g('+'), n('+'))*dS)) < 6e-14
