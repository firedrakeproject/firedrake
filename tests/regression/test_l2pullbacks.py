...
def get_complex(family, hdegree, vdegree=None):
    if family == "RT":
        ...
        hdivelem = HDiv(TensorProductElement(h1_horiz, l2_vert)) + HDiv(
            TensorProductElement(l2_horiz, h1_vert)
        )
...
def helmholtz_mixed(
    r, meshtype, family, hdegree, vdegree=None, meshd=None, useaction=False
):
...
def test_firedrake_helmholtz_mixed_l2pullbacks(
    family, degree, celltype, action, threshold
):
    diff = np.array(
        [
            helmholtz_mixed(r, "planar-" + celltype, family, degree, useaction=action)
            for r in range(3, 6)
        ]
    )
...
def test_firedrake_helmholtz_mixed_l2pullbacks_sphere(
    family, degree, celltype, md, action, threshold
):
    diff = np.array(
        [
            helmholtz_mixed(
                r, "spherical-" + celltype, family, degree, meshd=md, useaction=action
            )
            for r in range(2, 5)
        ]
    )
...
@pytest.mark.parametrize(
    ("hdegree", "vdegree", "threshold", "action"),
    [
        (1, 1, 1.9, False),
        (2, 1, 1.9, False),
        (1, 2, 1.9, False),
...
def test_firedrake_helmholtz_mixed_l2pullbacks_extruded(
    hdegree, vdegree, threshold, action
):
    diff = np.array(
        [
            helmholtz_mixed(
                i, "extruded", "ext", hdegree, vdegree=vdegree, useaction=action
            )
            for i in range(3, 6)
        ]
    )
...
def laplacian_IP(r, degree, meshd, meshtype):
    ...
    ddx = CV / FA
    ddx_avg = (ddx("+") + ddx("-")) / 2.0
    alpha = Constant(4.0 * degree * (degree + 1.0))
    gamma = Constant(8.0 * degree * (degree + 1.0))
    ...
    aIF = (
        inner(jump(u, n), jump(v, n)) * penalty_int
        - inner(avg(grad(u)), jump(v, n))
        - inner(jump(u, n), avg(grad(v)))
    ) * dIF  # interior facet term
    aEF = (
        inner(u, v) * penalty_ext - inner(grad(u), v * n) - inner(u * n, grad(v))
    ) * dEF  # exterior facet term
    a = aV + aEF + aIF
    L = inner(f, v) * dx
...
@pytest.mark.parametrize(
    ("degree", "meshd", "meshtype", "threshold"),
    [
        (1, 1, "planar-quad", 1.8),
        (2, 1, "planar-quad", 2.9),
        (1, 1, "planar-tri", 1.5),
