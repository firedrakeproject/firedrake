...
def test_facet_avg(degree, mesh):
    ...
    a = test*trial*ds + avg(test)*avg(trial)*dS
    l = facet_avg(source)*test*ds + avg(facet_avg(source))*avg(test)*dS
    ...
    v = TestFunction(Vt0)
    u = TrialFunction(Vt0)
    a0 = u*v*ds + avg(u)*avg(v)*dS
    L0 = source*v*ds + avg(source)*avg(v)*dS
    solve(a0 == L0, ft_ref_p0, solver_parameters={"pc_type": "lu", "ksp_type": "preonly"})

    l_ref = ft_ref_p0*test*ds + avg(ft_ref_p0)*avg(test)*dS
