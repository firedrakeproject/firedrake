...
def test_star_equivalence(problem_type, backend):
    ...
    if problem_type == "scalar":
        ...
        base = UnitSquareMesh(
            5, 5, distribution_parameters=distribution_parameters, quadrilateral=True
        )
        ...
        bcs = DirichletBC(V.sub(0), Constant((1.0, 0.0)), "on_boundary")
    ...
    star_solver = NonlinearVariationalSolver(
        nvproblem, solver_parameters=star_params, nullspace=nsp
    )
    star_solver.solve()
    star_its = star_solver.snes.getLinearSolveIterations()

    u.assign(0)
    comp_solver = NonlinearVariationalSolver(
        nvproblem, solver_parameters=comp_params, nullspace=nsp
    )
    comp_solver.solve()
...
def test_vanka_equivalence(problem_type):
    ...
    nvproblem = NonlinearVariationalProblem(a, u, bcs=bcs)
    star_solver = NonlinearVariationalSolver(
        nvproblem, solver_parameters=vanka_params, nullspace=nsp
    )
    star_solver.solve()
    star_its = star_solver.snes.getLinearSolveIterations()
