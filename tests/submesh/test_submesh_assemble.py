...
def test_submesh_assemble_cell_cell_integral_cell():
    ...
    subm = Submesh(mesh, dmcommon.CELL_SETS_LABEL, 999)
...
def test_submesh_assemble_cell_cell_cell_cell_integral_various():
    ...
    x, y = SpatialCoordinate(mesh)
    mesh_l = Submesh(mesh, dmcommon.CELL_SETS_LABEL, label_l)
    mesh_r = Submesh(mesh, dmcommon.CELL_SETS_LABEL, label_r)
    mesh_rl = Submesh(mesh_r, dmcommon.CELL_SETS_LABEL, label_rl)
...
def test_submesh_assemble_cell_cell_cell_cell_integral_avg():
    ...
    #x_l, y_l = SpatialCoordinate(mesh)
    mesh_r = Submesh(mesh, dmcommon.CELL_SETS_LABEL, label_r)
    x_r, y_r = SpatialCoordinate(mesh_r)
    #x_r, y_r = SpatialCoordinate(mesh)
    mesh_rl = Submesh(mesh_r, dmcommon.CELL_SETS_LABEL, label_rl)
    x_rl, y_rl = SpatialCoordinate(mesh_rl)
    #x_rl, y_rl = SpatialCoordinate(mesh)
    dx = Measure("dx", domain=mesh)
    dx_l = Measure("dx", domain=mesh_l)
    dx_r = Measure("dx", domain=mesh_r)
    ...
    #assert abs(assemble(facet_avg(y_l('+') * y_rl('|')) * ds_r(label_int)) - 1. / 3.) < 5.e-16
