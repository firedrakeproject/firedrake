...
def block_expr(request, A4, f4, f5):
...
def test_hybridisation():
    ...
    wA = Function(W)
    wB = Function(W)
    solve(a == L, wA, bcs=bcs, solver_parameters=params)
    solve(a == L, wB, bcs=bcs, solver_parameters=matfree_params)
    for a,b in zip([wA.dat.data], [wB.dat.data]):
        for c,d in zip(a,b):
