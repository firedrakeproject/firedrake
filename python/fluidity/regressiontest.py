...
class TestProblem:
    ...
    def __init__(self, filename, verbose=False, replace=None, backend=None,
                 no_pbs=False):
        ...
        for child in probtag.childNodes:
            ...
            if tag == "name":
                ...
                xmlcmd = child.getElementsByTagName(
                    "command_line")[0].childNodes[0].nodeValue
    ...
    def call_genpbs(self, dir):
        cmd = "genpbs \"" + self.filename[:-4] + "\" \"" + self.command_line \
            + "\" \"" + str(self.nprocs) + "\" \"" + self.random + "\""
        self.log("cd " + dir + "; " + cmd)
    ...
    def run(self, dir):
        ...
        try:
            ...
            backend = " BACKEND=" + self.backend if self.backend else ""
            self.log("Calling 'make input%s':" % backend)
            ret = os.system("cd " + dir + "; make input" + backend)
            assert ret == 0
        except OSError:
            self.log("No Makefile, not calling make")

        if (not self.no_pbs) and (self.nprocs > 1 or self.length == "long"):
